...
def add_foto(ruta_archivo, nombre_archivo, aviso_id):
    session = SessionLocal()
    foto = Foto(ruta_archivo=ruta_archivo, nombre_archivo=nombre_archivo, actividad_id=aviso_id)
    session.add(foto)
    session.commit()
    session.close()
...
def add_contacto(nombre, identificador, aviso_id):
    session = SessionLocal()
    contacto = ContactarPor(nombre=nombre, identificador=identificador, actividad_id=aviso_id)
